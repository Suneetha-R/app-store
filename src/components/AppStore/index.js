...
class AppStore extends Component {
  state = {actTabId: tabsList[0].tabId, searchInput: ''}

  onClickTabItem = tabId => {
    this.setState({actTabId: tabId})
  }
  ...
  filteredAppItems = () => {
    const {actTabId} = this.state
    const filteredAppsList = appsList.filter(
      eachApp => eachApp.category === actTabId,
    )
    return filteredAppsList
  }

  render() {
    const {actTabId, searchInput} = this.state
    const filteredApps = this.filteredAppItems()
          ...
          <ul className="tab-Items-container">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                isActive={eachItem.tabId === actTabId}
              />
            ))}
          </ul>
